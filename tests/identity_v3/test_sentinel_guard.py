...
def test_sentinels_are_rejected(scheme: str, value: str) -> None:
    assert is_sentinel_value(scheme, value) is True
...
def test_real_identifiers_are_not_sentinels(scheme: str, value: str) -> None:
    assert is_sentinel_value(scheme, value) is False


def test_sentinel_never_generates_auto_candidate() -> None:
...
def test_real_shared_id_still_generates_auto_candidate() -> None:
...
def test_accept_gate_blocks_sentinel_candidate_defence_in_depth() -> None:
    ...
    with pytest.raises(IdentityError, match="sentinel"):
